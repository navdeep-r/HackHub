...
// Add confetti particle component
...
const ConfettiAnimation = ({ isActive, onComplete }) => {
  ...
  useEffect(() => {
    ...
    const colors = ['#3b82f6', '#8b5cf6', '#60a5fa', '#a78bfa', '#93c5fd'];
    const newParticles = [];
    ...
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
      {particles.map((particle) => (
        <ConfettiParticle
          ...
          {...particle}
          onExit={() => {
            // Particle cleanup handled by parent component
          }}
        />
      ))}
    </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="relative w-full max-w-4xl max-h-[90vh]">
        <div className={modalCardClass}>
          {/* Gradient Header - Update styling to match view mode */}
          ...
          {/* Body: two-column layout - Update styling to match theme */}
          <div className="p-6 bg-gradient-to-br from-gray-900/50 to-gray-800/30 max-h-[70vh] overflow-y-auto">
            {error && (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2 space-y-6">
                {/* Update form styling */}
                <div className="rounded-xl bg-gradient-to-br from-blue-900/20 to-indigo-900/20 border border-blue-500/30 p-6">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 border border-blue-500/30 rounded-xl focus:ring-0 bg-blue-900/30 text-white shadow-sm transition-all placeholder-blue-300/50"
                    placeholder="Enter hackathon title"
                  />

                  <label className="block text-sm font-semibold text-white mb-2 mt-4">Description</label>
                  <textarea
                    name="description"
                    value={formData.description || ''}
                    onChange={handleFormChange}
                    className="w-full px-4 py-3 border border-blue-500/30 rounded-xl focus:ring-0 bg-blue-900/30 text-white transition-all resize-none shadow-sm placeholder-blue-300/50"
                    rows={5}
                ...
                <div className="rounded-xl bg-gradient-to-br from-blue-900/20 to-indigo-900/20 border border-blue-500/30 p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-semibold text-white mb-2">Registration Link</label>
                    <input
                      name="link"
                      type="url"
                      value={formData.link || ''}
                      onChange={handleFormChange}
                      className="w-full px-4 py-3 border border-blue-500/30 rounded-xl focus:ring-0 bg-blue-900/30 text-white transition-all shadow-sm placeholder-blue-300/50"
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={handleFormChange}
                      className="w-full px-4 py-3 border border-blue-500/30 rounded-xl focus:ring-0 bg-blue-900/30 text-white transition-all shadow-sm"
                    >
                      {CATEGORIES.map(cat => (
                ...
                <div className="rounded-xl bg-gradient-to-br from-blue-900/20 to-indigo-900/20 border border-blue-500/30 p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-semibold text-white mb-2">Registration Deadline</label>
                    <input
                      type="date"
                ...
                <div className="rounded-xl bg-gradient-to-br from-blue-900/20 to-indigo-900/20 border border-blue-500/30 p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-semibold text-white mb-2">Prize Pool</label>
                    <input
                      name="prizePool"
                      type="text"
              ...
              {/* Sidebar area (save/cancel + themes preview) - Update styling */}
              <aside className="space-y-6">
                <div className="p-4 rounded-xl bg-gradient-to-br from-blue-900/30 to-indigo-900/30 border border-blue-500/30 shadow-sm">
                  <h4 className="text-sm font-semibold text-white mb-3">Themes</h4>
                ...
                <div className="p-4 rounded-xl bg-gradient-to-br from-blue-900/30 to-indigo-900/30 border border-blue-500/30 shadow-sm">
                  <div className="mb-4">
                    <div className="text-sm text-blue-200">Registration Deadline</div>
                    <div className="font-semibold text-white">{formatDate(hackathon.registrationDeadline)}</div>
                  </div>

                  <div className="flex gap-3 justify-end">
                    <button
                      onClick={onClose}
                      className="px-4 py-2 border border-blue-500/30 rounded-lg bg-transparent text-white hover:bg-blue-900/30 transition-colors"
                    >
                      Cancel
                    </button>
                    <button
                      onClick={handleSave}
                      className="px-4 py-2 rounded-lg bg-gradient-to-r from-blue-600 to-indigo-600 text-white hover:opacity-90 transition-opacity"
                    >
                      Save Changes
                    </button>
                  </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="relative w-full max-w-6xl max-h-[90vh]">
        <div className={modalCardClass}>
          ...
          {/* Body */}
          <div className="p-6 bg-gradient-to-br from-gray-900/50 to-gray-800/30">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              {/* Left: Main Content */}
              <div className="lg:col-span-2 space-y-6">
                <div className="rounded-xl bg-gradient-to-br from-blue-900/20 to-indigo-900/20 shadow-sm border border-blue-500/30 p-6">
                  <h3 className="text-lg font-semibold text-white flex items-center gap-2 mb-3"><Layers size={18} className="text-blue-400" /> About this Hackathon</h3>
                  <p className="text-blue-100 leading-relaxed">{hackathon.description || 'No description provided.'}</p>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="p-4 rounded-lg bg-gradient-to-br from-blue-900/30 to-indigo-900/30 border border-blue-500/30 flex items-center gap-4">
                    <div className="p-3 bg-blue-500/20 rounded-lg shadow-sm">
                  ...
                  <div className="p-4 rounded-lg bg-gradient-to-br from-blue-900/30 to-indigo-900/30 border border-blue-500/30 flex items-center gap-4">
                    <div className="p-3 bg-indigo-500/20 rounded-lg shadow-sm">
                      <Clock className="text-indigo-400" size={20} />
                    </div>
                    <div>
                      <div className="text-sm text-blue-200">Event Date</div>
                      <div className="font-semibold text-white">{formatDate(hackathon.eventDate)}</div>
                    </div>
                  </div>
                </div>
                ...
                {/* Registered Students */}
                {role === 'faculty' && (
                  <div className="rounded-xl bg-gradient-to-br from-blue-900/20 to-indigo-900/20 border border-blue-500/30 shadow-sm overflow-hidden">
                    <div className="sticky top-0 bg-gradient-to-r from-blue-800/40 to-indigo-800/40 backdrop-blur-sm p-4 border-b border-blue-500/30 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <UserCheck className="text-blue-400" size={18} />
                        <div>
                          <div className="text-sm font-semibold text-white">Registered Students</div>
                          <div className="text-xs text-blue-200">{analytics?.registrationStats?.total || 0} total</div>
                        </div>
                      </div>
                      ...
                      {filteredStudents.length > 0 && (
                        <button 
                          ...
                          className="px-3 py-1 text-sm bg-gradient-to-r from-blue-600 to-indigo-600 text-white border border-blue-400/50 rounded-md hover:opacity-90 transition-opacity"
                        >
                          Export CSV
                        </button>
                      )}
                    ...
                    <div className="max-h-72 overflow-y-auto">
                      {loadingAnalytics && (
                        <div className="p-6 text-center"><Loader2 className="animate-spin text-blue-400" size={28} /></div>
                      )}

                      {!loadingAnalytics && filteredStudents.length === 0 && (
                        <div className="p-6 text-center text-blue-200">No registrations yet.</div>
                      )}
                        ...
                        <div className="divide-y divide-blue-500/20">
                          {pagedStudents.map((r, idx) => (
                            <div 
                              ...
                              className={`${idx % 2 === 0 ? 'bg-blue-900/10' : 'bg-indigo-900/10'} p-4 flex items-center justify-between` }
                            >
                              <div>
                                <div className="font-medium text-white">{r.student?.name || 'Unknown'}</div>
                                <div className="text-sm text-blue-200">{r.student?.email || ''}</div>
                              </div>
                    ...
                    {totalPages > 1 && (
                      <div className="p-3 flex items-center justify-between border-t border-blue-500/30">
                        <div className="text-sm text-blue-200">Page {currentPage} of {totalPages}</div>
                        <div className="flex gap-2">
                          <button 
                            onClick={() => setPage(p => Math.max(1, p - 1))} 
                            disabled={currentPage === 1} 
                            className="px-3 py-1 border border-blue-500/30 rounded-md text-sm text-white bg-blue-900/30 hover:bg-blue-800/50 disabled:opacity-50"
                          >
                            Prev
                          </button>
                          <button 
