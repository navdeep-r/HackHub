...
import { 
  ...
  Calendar, ExternalLink, Bell, Search, CheckCircle, Flame, BookOpen, Layers, Zap, Star
} from 'lucide-react';
...
const FacultyDashboard = () => {
  ...
  const impressionsCount = useCountUp(hackathons.reduce((sum, h) => sum + h.impressions, 0));
  const registrationsCount = useCountUp(hackathons.reduce((sum, h) => sum + h.registrations, 0));
